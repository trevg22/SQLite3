...
bool SQLiteDB::UpdateTableSchema(SQLiteTable &table) {
  const std::string SQLTableInfo = "pragma table_info(" + table.GetName() + ")";
...
// rename to colNames
std::string SQLiteDB::BuildSelect(const SQLiteTable &table,
                                  const std::vector<std::string> &columNames) {
...
SQLiteDB::GetDistinctColumnValues(const SQLiteTable &table,
  ...
  const std::string SQLSelectDist = "SELECT DISTINCT " + columnName + " FROM " +
                                    table.GetName() + BuildWhere(table);
  sqlite3_stmt *stmt = nullptr;
...
double SQLiteDB::GetDBColumn(const SQLiteTable &table,
                             const std::string &columnName) {
  std::string SQLSelect =
      "SELECT " + columnName + " FROM " + table.GetName() + " ";
  bool isFirst = true;
  for (const std::pair<std::string, WhereClause> clause : filters) {
