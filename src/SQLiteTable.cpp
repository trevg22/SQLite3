...
void SQLiteTable::InsertEle(const std::string& name, const std::string& type, const std::string &defVal)
{
	table.insert(std::map<std::string, DBColumn>::value_type(name, { type,defVal }));
	keyStrUpdated = false;
}
...
void SQLiteTable::SetName(const std::string& name)
{
	tableName = name;
}
const std::string SQLiteTable::GetColumnType(const std::string & colName ) 
{
  return table[colName].type;
}
const std::string& SQLiteTable::GetName() const {
...
//Note can speed up by not storing values in map at all and just writing them to DB
void MavTable::SetValues(double time, const std::string& resp, size_t cell, double val)
{
